...
func unify(_ c: ConstraintSet) -> Substitution {
    guard let head = c.equations.first else {
        return Substitution()
    }
    ...
    case (.typeVar(let a), .typeVar(let b)) where a == b:
        let equations = Array(c.equations.dropFirst())
    ...
    case (.typeVar, _) where !T.typeVars.contains(S):
        ...
        let cc = ConstraintSet(equations: Array(c.equations.dropFirst()))
    ...
    case (.arrow(let s1, let s2), .arrow(let t1, let t2)):
        var e = Array(c.equations.dropFirst())
